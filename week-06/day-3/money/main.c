...
int main()
{
    ...
    int *relativesAtParty = (int *) calloc(relatives, sizeof(int));
    while (heritage != 0) {
        int randomRelative = rand() % relatives;
        if (relativesAtParty[randomRelative] == 0) {
            relativesAtParty[randomRelative] += heritage / 2;
            heritage /= 2;
        }
    }
    for (int i = 0; i < relatives; i++) {
        printf("%d,", relativesAtParty[i]);
    }
    return 0;
}